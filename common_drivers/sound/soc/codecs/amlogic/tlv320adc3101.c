...
static int adc3101_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	iface_reg_1 = snd_soc_component_read(component, ADC3101_IFACE1);
	iface_reg_1 = iface_reg_1 & ~(3 << 6 | 3 << 2);
	dsp_a_val = snd_soc_component_read(component,
					   ADC3101_DATASLOTOFFSETCTL);
	dsp_a_val = 0;
	iface_reg_2 = snd_soc_component_read(component, ADC3101_IFACE3);
	...
	snd_soc_component_write(component, ADC3101_DATASLOTOFFSETCTL,
				dsp_a_val);
			     ...
			     struct snd_soc_dai *codec_dai)
{
	...
	snd_soc_component_write(component, ADC3101_PLLDMSB,
				(adc3101_divs[i].pll_d >> 8));
	snd_soc_component_write(component, ADC3101_PLLDLSB,
				(adc3101_divs[i].pll_d & 0xff));
	/* NADC divider value */
	data = snd_soc_component_read(component, ADC3101_NADC);
	data &= ~(0x7f);
	data |= 0x80;
	snd_soc_component_write(component, ADC3101_NADC,
				data | adc3101_divs[i].nadc);

	pr_info("%s NADC = 0x%02x\n",
		__func__,
		snd_soc_component_read(component, ADC3101_NADC));

	/* MADC divider value */
	data = snd_soc_component_read(component, ADC3101_MADC);
	data &= ~(0x7f);
	data |= 0x80;
	snd_soc_component_write(component, ADC3101_MADC,
				data | adc3101_divs[i].madc);

	pr_info("%s MADC = 0x%02x\n",
		__func__,
		snd_soc_component_read(component, ADC3101_MADC));

	/* AOSR value */
	snd_soc_component_write(component,
				ADC3101_AOSR, adc3101_divs[i].aosr);
	pr_info("%s AOSR=%02x\n",
		__func__,
		snd_soc_component_read(component, ADC3101_AOSR));

	data = snd_soc_component_read(component, ADC3101_IFACE1);
	data = data & ~(3 << 4);
	...
	if (adc3101->differential_pair == 1) {
		...
		snd_soc_component_write(component,
					ADC3101_LMICPGANIN, 0x33); /* 54 */
		snd_soc_component_write(component,
					ADC3101_RMICPGANIN, 0x33); /* 57 */
		snd_soc_component_write(component,
					ADC3101_LMICPGAPIN, 0x3F); /* 52 */
		snd_soc_component_write(component,
					ADC3101_RMICPGAPIN, 0x3F); /* 55 */
	} else {
		pr_info("%s single end\n", __func__);
		snd_soc_component_write(component,
					ADC3101_LMICPGANIN, 0x3F); /* 54 */
		snd_soc_component_write(component,
					ADC3101_RMICPGANIN, 0x3F); /* 57 */
		snd_soc_component_write(component,
					ADC3101_LMICPGAPIN, 0xCF); /* 52 */
		snd_soc_component_write(component,
					ADC3101_RMICPGAPIN, 0xCF); /* 55 */
	}
	...
	pr_info("%s ADCSETUP = %02x\n",
		__func__,
		snd_soc_component_read(component, ADC3101_ADCSETUP));
	pr_info("%s DOUTCTL=%02x\n",
		__func__,
		snd_soc_component_read(component, ADC3101_DOUTCTL));
	pr_info("%s MICBIAS=%02x\n",
		__func__,
		snd_soc_component_read(component, ADC3101_MICBIAS));
...
static int adc3101_mute(struct snd_soc_dai *dai, int mute)
{
	...
	dac_reg = snd_soc_component_read(component, ADC3101_DACMUTE)
		& ~ADC3101_MUTEON;
	if (mute)
		snd_soc_component_write(component, ADC3101_DACMUTE,
					dac_reg | ADC3101_MUTEON);
	else
...
static int adc3101_mute_stream(struct snd_soc_dai *dai, int mute, int stream)
{
	return adc3101_mute(dai, mute); // ignore stream
}
...
static const struct snd_soc_dai_ops adc3101_ops = {
	...
	.mute_stream  = adc3101_mute_stream,
...
static int adc3101_codec_probe(struct snd_soc_component *component)
{
	 ...
	 */
	tmp_reg = snd_soc_component_read(component, ADC3101_ADCSETUP);
			     ...
			     const struct i2c_device_id *id)
{
	...
	adc3101 = devm_kzalloc(&i2c->dev,
			       sizeof(struct adc3101_priv),
			       GFP_KERNEL);
	if (!adc3101)
		return -ENOMEM;
