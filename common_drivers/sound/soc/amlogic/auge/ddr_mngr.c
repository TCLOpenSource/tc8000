	...
	bool enable)
{
	...
	pr_info("aml_vad_enable, enter\n");
	if (!to)
		return;

	pr_info("aml_vad_enable, call vad_set_toddr_info\n");
	vad_set_toddr_info(enable ? to : NULL);

	/* vad enable or not */
	pr_info("aml_vad_enable, call vad_enable\n");
	vad_enable(enable);
	pr_info("aml_vad_enable, exit\n");
}
...
static int aml_ddr_mngr_platform_probe(struct platform_device *pdev)
{
	...
	pr_warn("[%s] probe 202307011037_amlsz624\n", __func__);
