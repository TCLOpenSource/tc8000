...
#undef pr_fmt
#define pr_fmt(fmt) "t5m_audio_clocks: " fmt
...
static struct clk_gate *t5m_audio_clk_gates[] = {
	&audio_ddr_arb,
	&audio_pdm,
	&audio_tdmina,
	&audio_tdminb,
...
static struct clk_hw *t5m_audio_clk_hws[] = {
...
static int t5m_clk_gates_init(struct clk **clks, void __iomem *iobase)
{
	int clkid;

	if (ARRAY_SIZE(t5m_audio_clk_gates) != MCLK_BASE) {
	...
	for (clkid = 0; clkid < MCLK_BASE; clkid++) {
		unsigned long offset =
			(unsigned long)t5m_audio_clk_gates[clkid]->reg;
		t5m_audio_clk_gates[clkid]->reg =
			(void __iomem *)((unsigned long)iobase + offset);
		clks[clkid] = clk_register(NULL, t5m_audio_clk_hws[clkid]);
...
static int t5m_clks_init(struct clk **clks, void __iomem *iobase)
{
...
struct audio_clk_init t5m_audio_clks_init = {
	.clk_num   = NUM_AUDIO_CLKS,
	.clk_gates = t5m_clk_gates_init,
	.clks      = t5m_clks_init,
};
