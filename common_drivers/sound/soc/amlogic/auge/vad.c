...
static int vad_freeze_thread(void *data)
{
	...
	pr_info("vad: freeze thread start__1__\n");

	if (!p_vad) {
		pr_info("vad: freeze thread end__2__\n");
		return 0;
	}

	current->flags |= PF_NOFREEZE;
	pr_info("vad: freeze thread start__3__\n");
...
static void vad_deinit(struct vad *p_vad)
{
	...
	if (p_vad->level == LEVEL_KERNEL) {
		...
		kfree(p_vad->buf);
		p_vad->buf = NULL;
		pr_info("%s, exit sub", __func__);
	}

	/* free irq */
	pr_info("%s, call free_irq", __func__);
	free_irq(p_vad->irq_wakeup, p_vad);
	free_irq(p_vad->irq_fs, p_vad);

	/* clock disabled */
	pr_info("%s, call vad_set_clks", __func__);
...
void vad_update_buffer(bool isvadbuf)
{
	...
	if (p_vad->chipinfo &&
		...
		curr_addr = aml_toddr_get_position(p_vad->tddr);
		pr_info("%s line:%d curr_addr:%p start:%p end:%p\n", __func__, __LINE__, curr_addr, start, end);
...
//#if (defined CONFIG_VAD_WAKEUP_ASR)
//#if (1)
		pr_info("%s, czy_inifdefine", __func__);
                aml_asr_enable();
//#endif
                pr_info("%s, czy_endifdefine", __func__);
                vad_init(p_vad);
		aml_set_vad(p_vad->en, p_vad->src);
	} else {
		pr_info("%s, call aml_set_vad", __func__);
		aml_set_vad(p_vad->en, p_vad->src);
		pr_info("%s, call vad_deinit", __func__);
		vad_deinit(p_vad);
//#if (defined CONFIG_VAD_WAKEUP_ASR)
//#if (1)
		pr_info("%s, call aml_asr_disable", __func__);
		aml_asr_disable();
		pr_info("%s, end else", __func__);
//#endif
	}
