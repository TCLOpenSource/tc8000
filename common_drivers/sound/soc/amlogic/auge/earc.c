...
static void tx_hold_bus_work_func(struct work_struct *p_work)
{
	...
	if (p_earc && p_earc->tx_dmac_clk_on)
		earctx_dmac_hold_bus_and_mute(p_earc->tx_dmac_map);
}
...
static void earctx_init(int earc_port, bool st)
{
	...
	if (!st) {
		...
		if (p_earc->tx_dmac_clk_on)
			earctx_biphase_work_clear(p_earc->tx_dmac_map);
	} else {
				       ...
				       bool is_earc, bool state)
{
	...
	if (state) {
		...
		if (p_earc->tx_arc_status != ATNDTYP_DISCNCT) {
...
static int earc_platform_probe(struct platform_device *pdev)
{
	...
	dev_err(dev, "719 registered eARC platform\n");
