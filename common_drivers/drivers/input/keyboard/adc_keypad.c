...
// #ifdef AMLOGIC_STR
static int adc_keypad_suspend_state_notifier(struct notifier_block *nb,
				unsigned long event,
				void *dummy)
{
	if (event == SUSPEND_STATE_INPUT_DISABLE)
		keypad_enable_flag =  false;
	else if (event == SUSPEND_STATE_INPUT_ENABLE)
		keypad_enable_flag =  true;

	pr_info("%s %d event = %d\n", __func__, __LINE__, event);
	return NOTIFY_OK;
}

static struct notifier_block adc_keypad_notif_block = {
	.notifier_call = adc_keypad_suspend_state_notifier,
};
// #endif AMLOGIC_STR
		...
		if (kp->count < KEY_JITTER_COUNT) {
			...
			if (keypad_enable_flag && kp->report_code != code) {
				...
				if (code == KEY_POWER) {
					...
					//set keypad wakeup source as 6
					if (is_pm_s2idle_mode())
						set_wakeup_reason(POWER_KEY_WAKEUP);
				}
				kp->report_code = code;
			}
			kp->count = 0;
...
static int meson_adc_kp_probe(struct platform_device *pdev)
{
	...
	mod_delayed_work(system_wq, &kp->work,
			 msecs_to_jiffies(kp->poll_period));
	// #ifdef AMLOGIC_STR
	register_suspend_state_notifier(&adc_keypad_notif_block);
	// #endif AMLOGIC_STR
	return ret;
...
static int meson_adc_kp_remove(struct platform_device *pdev)
{
	...
	// #ifdef AMLOGIC_STR
	unregister_suspend_state_notifier(&adc_keypad_notif_block);
	// #endif AMLOGIC_STR
	class_unregister(&kp->kp_class);
