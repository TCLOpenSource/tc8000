...
static int wake_event;
static ssize_t wake_event_show(struct device *dev,
				 struct device_attribute *attr,
				 char *buf)
{
	return sprintf(buf, "%d\n", wake_event);
...
static ssize_t wake_event_store(struct device *dev,
			   struct device_attribute *attr,
			   const char *buf, size_t count)
{
	int ret, value;
	...
	wake_event = value;
	if (wake_event == 1 && r_dev->input_device) {
		pr_info("%s %d\n", __func__, __LINE__);
