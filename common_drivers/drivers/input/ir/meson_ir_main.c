...
// #ifdef AMLOGIC_STR
static struct meson_ir_chip *global_chip = NULL;

static int ir_suspend_state_notifier(struct notifier_block *nb,
				unsigned long event,
				void *dummy)
{
	struct meson_ir_chip *chip = global_chip;

	if (!chip)
		return NOTIFY_OK;

	if (event == SUSPEND_STATE_INPUT_DISABLE)
		chip->set_register_config(chip, 2);
	else if (event == SUSPEND_STATE_INPUT_ENABLE)
...
static struct notifier_block ir_notif_block = {
	.notifier_call = ir_suspend_state_notifier,
};
// #endif AMLOGIC_STR
...
=======
>>>>>>> parent of 7d4f93b8... [T963D4CN-1] 工厂遥控器无法使用(2/2)
...
static int meson_ir_probe(struct platform_device *pdev)
{
	...
	// #ifdef AMLOGIC_STR
	global_chip = chip;
	register_suspend_state_notifier(&ir_notif_block);
	// #endif AMLOGIC_STR
	return 0;
}
...
static int meson_ir_remove(struct platform_device *pdev)
{
	struct meson_ir_chip *chip = platform_get_drvdata(pdev);
	// #ifdef AMLOGIC_STR
	unregister_suspend_state_notifier(&ir_notif_block);
	global_chip = NULL;
	// #endif AMLOGIC_STR
...
static int meson_ir_resume(struct device *dev)
{
	...
	if (get_resume_method() == REMOTE_CUS_WAKEUP) {
		input_event(chip->r_dev->input_device, EV_KEY, KEY_POWER, 1);
		input_sync(chip->r_dev->input_device);
		input_event(chip->r_dev->input_device, EV_KEY, KEY_POWER, 0);
		input_sync(chip->r_dev->input_device);
	}
#endif
