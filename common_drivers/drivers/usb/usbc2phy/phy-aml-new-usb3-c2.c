...
struct usb_aml_regs_v2 usb_c2_new_aml_regs_v2;
struct amlogic_usb_v2	*g_phy_c2;
...
static void usb3_c2_aml_new_usb2_init(void)
{
	...
	reg_addr = (unsigned long)g_phy_c2->usb2_phy_cfg;
	if (g_phy_c2->otg == 2) {
		id_gpio_status = gpiod_get_value(g_phy_c2->idgpiodesc);
		//printk(KERN_ERR"<%s>id_gpio_status = %d\n",
			 //  __func__, id_gpio_status);

		if (id_gpio_status == 1) {
			amlogic_new_set_vbus_power(g_phy_c2, 1);
			aml_new_usb_notifier_call(0);
			set_mode(reg_addr, HOST_MODE);
		} else {
			set_mode(reg_addr, DEVICE_MODE);
			aml_new_usb_notifier_call(1);
			amlogic_new_set_vbus_power(g_phy_c2, 0);
		}
	} else {
		r5.d32 = readl(usb_c2_new_aml_regs_v2.usb_r_v2[5]);
...
static int amlogic_new_usb3_init(struct usb_phy *x)
{
	...
	for (i = 0; i < 7; i++) {
		usb_c2_new_aml_regs_v2.usb_r_v2[i] = (void __iomem *)
			((unsigned long)phy->regs + 4 * i);
	}

	r1.d32 = readl(usb_c2_new_aml_regs_v2.usb_r_v2[1]);
	r1.b.u3h_fladj_30mhz_reg = 0x20;
	writel(r1.d32, usb_c2_new_aml_regs_v2.usb_r_v2[1]);

	r5.d32 = readl(usb_c2_new_aml_regs_v2.usb_r_v2[5]);
	r5.b.iddig_en0 = 1;
	r5.b.iddig_en1 = 1;
	r5.b.iddig_th = 255;
	writel(r5.d32, usb_c2_new_aml_regs_v2.usb_r_v2[5]);

	r6.d32 = readl(usb_c2_new_aml_regs_v2.usb_r_v2[6]);
	r6.b.vbusdig_en0 = 1;
	r6.b.vbusdig_en1 = 1;
	r6.b.vbusdig_th = 255;
	writel(r6.d32, usb_c2_new_aml_regs_v2.usb_r_v2[6]);

	/* config usb3 phy */
	if (phy->phy.flags == AML_USB3_PHY_ENABLE) {
		r3.d32 = readl(usb_c2_new_aml_regs_v2.usb_r_v2[3]);
		r3.b.p30_ssc_en = 1;
		r3.b.p30_ssc_range = 2;
		r3.b.p30_ref_ssp_en = 1;
		writel(r3.d32, usb_c2_new_aml_regs_v2.usb_r_v2[3]);
		udelay(2);
		r2.d32 = readl(usb_c2_new_aml_regs_v2.usb_r_v2[2]);
		r2.b.p30_pcs_tx_deemph_3p5db = 0x15;
		r2.b.p30_pcs_tx_deemph_6db = 0x20;
		writel(r2.d32, usb_c2_new_aml_regs_v2.usb_r_v2[2]);
		udelay(2);
		r1.d32 = readl(usb_c2_new_aml_regs_v2.usb_r_v2[1]);
...
static void amlogic_gxl_work(struct work_struct *work)
{
	...
	if (r5.b.iddig_curr == 0) {
		...
		aml_new_usb_notifier_call(0);
		set_usb_phy_reg10(HOST_MODE);
		set_mode(reg_addr, HOST_MODE);
	} else {
		set_mode(reg_addr, DEVICE_MODE);
		aml_new_usb_notifier_call(1);
		set_usb_phy_reg10(DEVICE_MODE);
...
static int amlogic_new_C2_usb3_v2_probe(struct platform_device *pdev)
{
	...
	phy->otg = otg;
	phy->usb2_phy_init = usb3_c2_aml_new_usb2_init;
...
{
	return platform_driver_register(&amlogic_new_c2_usb3_v2_driver);
}
#endif
