...
static int spinand_read_reg_op(struct spinand_device *spinand, u8 reg, u8 *val)
{
	...
	ret = meson_spi_mem_exec_op(spinand->spimem, &op);
	if (ret)
		return ret;
...
static int spinand_write_reg_op(struct spinand_device *spinand, u8 reg, u8 val)
{
	...
	*spinand->scratchbuf = val;
	return meson_spi_mem_exec_op(spinand->spimem, &op);
}
				     ...
				     const struct nand_page_io_req *req)
{
	...
	while (nbytes) {
		ret = meson_spi_mem_dirmap_write(wdesc, column, nbytes, buf);
		if (ret < 0)
			return ret;
			...
			u8 *s)
{
	...
	ret = meson_spi_mem_poll_status(spinand->spimem, &op, STATUS_BUSY, 0,
				  initial_delay_us,
				  poll_delay_us,
			      ...
			      u8 ndummy, u8 *buf)
{
	...
	dev_dbg(&spinand->spimem->spi->dev, "id: 0x%x 0x%x 0x%x 0x%x 0x%x\n",
