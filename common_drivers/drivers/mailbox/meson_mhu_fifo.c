				 ...
				 int idx, struct mbox_message *mbox_msg)
{
	...
	mhu_chan = mhu_get_channel(&cl, idx);
	if (IS_ERR_OR_NULL(mhu_chan)) {
		dev_err(dev, "Not have this chan\n");
		ret = PTR_ERR(mhu_chan);
		goto err_send3;
	}
	mutex_lock(&mhu_chan->mutex);
	chan = mbox_request_channel(&cl, idx);
	if (IS_ERR_OR_NULL(chan)) {
		mutex_unlock(&mhu_chan->mutex);
				  ...
				  size_t count, loff_t *ppos)
{
	...
	cl.tx_tout = MBOX_TIME_OUT;
	cl.rx_callback = NULL;
	mhu_chan = mhu_get_channel(&cl, send_channel);
	if (IS_ERR_OR_NULL(mhu_chan)) {
		dev_err(dev, "Not have this chan\n");
		ret = PTR_ERR(mhu_chan);
		goto err_write3;
...
static int mhu_fifo_probe(struct platform_device *pdev)
{
	...
	err = mbox_controller_register(&mhu_ctlr->mbox_con);
	if (err) {
...
static int mhu_fifo_remove(struct platform_device *pdev)
{
	...
	mhu_controller_unregister(ctlr);
