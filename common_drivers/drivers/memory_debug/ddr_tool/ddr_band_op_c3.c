...
static void c3_dmc_port_config(struct ddr_bandwidth *db, int channel, int port)
{
	unsigned int val;
	unsigned int off = 0;
	int subport = -1;
...
static void c3_dmc_range_config(struct ddr_bandwidth *db, int channel,
...
static unsigned long c3_get_dmc_freq_quick(struct ddr_bandwidth *db)
{
	unsigned int val;
...
static void c3_dmc_bandwidth_enable(struct ddr_bandwidth *db)
{
	unsigned int val;
...
static void c3_dmc_bandwidth_init(struct ddr_bandwidth *db)
{
	unsigned int i;
	/* set timer trigger clock_cnt */
	writel(db->clock_count, db->ddr_reg1 + DMC_MON_TIMER);
	c3_dmc_bandwidth_enable(db);

	for (i = 0; i < db->channels; i++) {
		if (!db->port[i])
			c3_dmc_port_config(db, i, -1);
		c3_dmc_range_config(db, i, 0, 0xffffffff);
		db->range[i].start = 0;
		db->range[i].end   = 0xffffffff;
	}
}

static int c3_handle_irq(struct ddr_bandwidth *db, struct ddr_grant *dg)
{
...
static int c3_dump_reg(struct ddr_bandwidth *db, char *buf)
{
	int s = 0, i;
	unsigned int r, off;
...
struct ddr_bandwidth_ops c3_ddr_bw_ops = {
	.init             = c3_dmc_bandwidth_init,
	.config_port      = c3_dmc_port_config,
	.config_range     = c3_dmc_range_config,
	.get_freq         = c3_get_dmc_freq_quick,
	.handle_irq       = c3_handle_irq,
	.bandwidth_enable = c3_dmc_bandwidth_enable,
#if DDR_BANDWIDTH_DEBUG
	.dump_reg         = c3_dump_reg,
#endif
};
