...
static size_t t5m_dmc_dump_reg(char *buf)
{
	int sz = 0, i, j;
...
static void __t5m_dmc_mon_irq(struct dmc_monitor *mon, void *io)
{
	unsigned long value;
...
static void t5m_dmc_mon_irq(struct dmc_monitor *mon, void *io)
{
	int i;

	if (io) {
		__t5m_dmc_mon_irq(mon, io);
	} else {
...
void t5m_dmc_mon_disable(struct dmc_monitor *mon)
{
	void *io;
	int i;
...
struct dmc_mon_ops t5m_dmc_mon_ops = {
	.handle_irq = t5m_dmc_mon_irq,
	.set_monitor = t5m_dmc_mon_set,
	.disable    = t5m_dmc_mon_disable,
	.dump_reg   = t5m_dmc_dump_reg,
};
