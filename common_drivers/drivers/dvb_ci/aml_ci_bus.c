 ...
 *   - -EINVAL : error
 */
static  int aml_ci_bus_mem_read(struct aml_ci *ci_dev, int slot, int addr)
{
	u8  data = 0;
	u16 address = addr;
	int value = 0;
 ...
 *   - -EINVAL : error
 */
static int aml_ci_bus_mem_write(struct aml_ci *ci_dev, int slot, int addr,  u8 val)
{
	u8  data = val;
	u16 address = addr;
	int value = 0;
 ...
 *   - -EINVAL : error
 */
static int aml_ci_bus_io_read(struct aml_ci *ci_dev, int slot, int addr)
{
	u8  data = 0;
	u16 address = addr;
	int value = 0;
 ...
 *   - -EINVAL : error
 */
static int aml_ci_bus_io_write(struct aml_ci *ci_dev, int slot, int addr, u8 val)
{
	u8  data = val;
	u16 address = addr;
	int value = 0;
 ...
 *   - -EINVAL : error
 */
static int aml_ci_bus_rst(struct aml_ci *ci_dev, int slot, int level)
{
	int value = 0;
	u32 ctrl = 0;
	struct aml_ci_bus *ci_bus_dev = ci_dev->data;
...
static void aml_ci_bus_full_test(struct aml_ci *ci_dev)
{
	...
	if (1) {
		...
		while (1) {
			mdelay(20);
			count1--;
			reg1 = aml_ci_bus_io_read(ci_dev, 0, COM_STA_REG);
			if (FR != (FR & reg1)) {
				continue;
			} else {
				pr_dbg("CAM Reset Ok\r\n");
 ...
 * @param slot id.
 * @param address to read from. Updated.
 ...
 */
static int dvb_ca_en50221_read_tuple(int *address,
