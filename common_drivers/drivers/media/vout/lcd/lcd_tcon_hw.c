			     ...
			     struct tcon_mem_map_table_s *mm_table)
{
	...
	if (mm_table->block_flag & LCD_TCON_DATA_VALID_OD) {
		if (mm_table->block_flag & LCD_TCON_DATA_BLOCK_CTRL_OD_EN_DELAY) {
			lcd_tcon_sm_od_cnt = 0;
			local_cfg->sm_state = TCON_STATE_OD_PRE_EN;
		}
	}
...
int lcd_tcon_enable_tl1(struct aml_lcd_drv_s *pdrv)
{
	...
	if (mm_table->version == 0) {
		if (mm_table->block_flag & LCD_TCON_DATA_VALID_VAC) {
			if (mm_table->block_flag & LCD_TCON_DATA_VALID_DEMURA)
				flag = 1;
			else
				flag = 0;
		...
		if (mm_table->block_flag & LCD_TCON_DATA_VALID_ACC)
