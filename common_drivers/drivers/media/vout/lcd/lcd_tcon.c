...
int lcd_tcon_info_print(char *buf, int offset)
{
	...
	n = lcd_debug_info_len(len + offset);
	len += snprintf((buf + len), n,
		"\ntcon_data block_flag: 0x%08x\n", tcon_mm_table.block_flag);
	return len;
}
...
unsigned int lcd_tcon_sm_od_cnt;
static void lcd_tcon_sm_handler(struct aml_lcd_drv_s *pdrv)
{
	switch (tcon_local_cfg.sm_state) {
	case TCON_STATE_OD_PRE_EN:
		if (lcd_tcon_sm_od_cnt++ >= 240) {
			tcon_local_cfg.sm_state = TCON_STATE_OD_DDRIF_EN;
			lcd_tcon_sm_od_cnt = 0;
		}
		break;
	case TCON_STATE_OD_DDRIF_EN:
		lcd_tcon_setb(pdrv, 0x263, 1, 31, 1);
		if (lcd_tcon_sm_od_cnt++ >= 2) {
			tcon_local_cfg.sm_state = TCON_STATE_OD_DEMO_OFF;
			lcd_tcon_sm_od_cnt = 0;
		}
		break;
	case TCON_STATE_OD_DEMO_OFF:
		lcd_tcon_setb(pdrv, 0x240, 0, 1, 1);
		if (lcd_tcon_sm_od_cnt++ >= 2) {
			tcon_local_cfg.sm_state = TCON_STATE_OD_EN;
			lcd_tcon_sm_od_cnt = 0;
		}
		break;
	case TCON_STATE_OD_EN:
		tcon_local_cfg.sm_state = TCON_STATE_DONE;
		break;
	case TCON_STATE_DONE:
		break;
	}
}
...
void lcd_tcon_vsync_isr(struct aml_lcd_drv_s *pdrv)
{
	...
	if (tcon_conf->tcon_is_busy)
		return;

	lcd_tcon_sm_handler(pdrv);
...
static void lcd_tcon_data_complete_check(struct aml_lcd_drv_s *pdrv, int index)
{
	...
	if (pdrv->data->chip_type == LCD_CHIP_TL1 ||
		...
		if (tcon_mm_table.demura_cnt < 2) {
			tcon_mm_table.block_flag &= ~LCD_TCON_DATA_VALID_DEMURA;
			if (table) {
				/* disable demura */
				table[0x178] = 0x38;
				table[0x17c] = 0x20;
...
int lcd_tcon_data_load(struct aml_lcd_drv_s *pdrv, unsigned char *data_buf, int index)
{
	...
	unsigned int priority, valid_flag;
	struct lcd_tcon_config_s *tcon_conf = get_lcd_tcon_config();
	...
	if (block_header->block_type != LCD_TCON_DATA_BLOCK_TYPE_BASIC_INIT) {
		tcon_mm_table.block_flag |= block_header->block_flag;
		valid_flag = block_header->block_flag & LCD_TCON_DATA_VALID_MASK;
		if (valid_flag  == LCD_TCON_DATA_VALID_DEMURA)
	...
	pdrv->tcon_status = tcon_mm_table.block_flag & LCD_TCON_DATA_VALID_MASK;
...
int lcd_tcon_bin_load(struct aml_lcd_drv_s *pdrv)
{
	...
	if (tcon_mm_table.version == 0) {
		...
		if (pdrv->data->chip_type == LCD_CHIP_TL1 ||
			...
			if (ret == 0)
				tcon_mm_table.block_flag |= LCD_TCON_DATA_VALID_VAC;
		...
		if (ret == 0)
			tcon_mm_table.block_flag |= LCD_TCON_DATA_VALID_ACC;
...
static void lcd_tcon_axi_mem_config_t5(void)
{
	...
	if (!tcon_rmem.axi_rmem)
		return;
	lcd_tcon_conf->axi_reg = kcalloc(lcd_tcon_conf->axi_bank, sizeof(unsigned int), GFP_KERNEL);
