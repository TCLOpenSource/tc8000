...
void lcd_mipi_dsi_config_set(struct aml_lcd_drv_s *pdrv)
{
	...
	if (dconf->bit_rate_max == 0) { /* auto calculate */
		...
		if (dconf->local_bit_rate_min > dconf->local_bit_rate_max) {
			LCDPR("[%d]: %s: bit_rate_max %d can't reach bandwidth requirement %d\n",
				pdrv->index, __func__, dconf->local_bit_rate_max,
				dconf->local_bit_rate_min);
			dconf->local_bit_rate_min = dconf->local_bit_rate_max - pclk;
		}
	}
	if (lcd_debug_print_flag & LCD_DBG_PR_NORMAL) {
		LCDPR("[%d]: %s: local_bit_rate_max=%u, local_bit_rate_min=%u\n",
		      pdrv->index, __func__,
