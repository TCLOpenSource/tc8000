...
static int lcd_resume(struct platform_device *pdev)
{
	...
	pr_emerg("%s 1 drv->status=0x%x resume_flag=0x%x\n", __func__,
		pdrv->status, pdrv->resume_flag);
	if ((pdrv->status & LCD_STATUS_VMODE_ACTIVE) == 0)
		return 0;

	pr_emerg("%s 2 drv->status=0x%x resume_flag=0x%x\n", __func__,
		pdrv->status, pdrv->resume_flag);
	mutex_lock(&lcd_power_mutex);
	LCDPR("[%d]: %s\n", pdrv->index, __func__);
	pdrv->resume_flag |= LCD_RESUME_PREPARE;
...
static int lcd_boot_ffv_freeze_setup(char *str)
{
	struct lcd_boot_ctrl_s *boot_ctrl = &lcd_boot_ctrl_config[0];
	char ffv_freeze[16];

	if (str)
		sprintf(ffv_freeze, "%s", str);

	pr_emerg("%s ffv_freeze: %s\n", __func__, ffv_freeze);
	if (strcmp(ffv_freeze, "on") == 0)
		boot_ctrl->ffv_freeze = 1;
	else
		boot_ctrl->ffv_freeze = 0;
	return 0;
}
...
__setup("ffv_freeze=", lcd_boot_ffv_freeze_setup);
