				...
				unsigned int mode, unsigned int ckdi)
{
	...
	if (phy->flag & (1 << 13)) {
		...
		for (i = 0; i < 12; i++) {
			phy->lane[i].amp = (chctl >> 3) & 0x7;
			if (lcd_debug_print_flag & LCD_DBG_PR_ADV)
				LCDPR("%s: %d\n", __func__, phy->lane[i].amp);
		}
	}
	lcd_ana_write(HHI_DIF_CSI_PHY_CNTL15, cntl15);
	lcd_ana_write(HHI_DIF_CSI_PHY_CNTL16, cntl16);
	amp = ((phy->lane[0].amp & 0x7) << 3 | (phy->lane[1].amp & 0x7) << 19);
	lcd_ana_write(HHI_DIF_CSI_PHY_CNTL8, chctl | amp);
	...
	lcd_ana_write(HHI_DIF_CSI_PHY_CNTL9, chctl | amp);
	...
	lcd_ana_write(HHI_DIF_CSI_PHY_CNTL10, chctl | amp);
	...
	lcd_ana_write(HHI_DIF_CSI_PHY_CNTL11, chctl | amp);
	...
	lcd_ana_write(HHI_DIF_CSI_PHY_CNTL12, chctl | amp);
	...
	lcd_ana_write(HHI_DIF_CSI_PHY_CNTL13, chctl | amp);
 ...
 *    ckdi: clk phase for minilvds
 */
static void lcd_phy_cntl_set_t5m(struct phy_config_s *phy, int status, int bypass,
				unsigned int mode, unsigned int ckdi)
{
	unsigned int cntl15 = 0, cntl16 = 0;
...
static void lcd_lvds_phy_set_t5m(struct aml_lcd_drv_s *pdrv, int status)
{
	struct phy_config_s *phy = &pdrv->config.phy_cfg;
	unsigned int cntl14 = 0;
	...
	if (status) {
		...
		lcd_phy_cntl_set_t5m(phy, status, 0, 1, 0);
	} else {
		lcd_phy_cntl_set_t5m(phy, status, 0, 0, 0);
	}
}

static void lcd_vbyone_phy_set_t5m(struct aml_lcd_drv_s *pdrv, int status)
{
	struct phy_config_s *phy = &pdrv->config.phy_cfg;
	unsigned int cntl14 = 0;
...
static void lcd_mlvds_phy_set_t5m(struct aml_lcd_drv_s *pdrv, int status)
{
	struct mlvds_config_s *mlvds_conf;
	struct phy_config_s *phy = &pdrv->config.phy_cfg;
	unsigned int ckdi, cntl14 = 0;
	...
	if (status) {
		...
		lcd_ana_write(ANACTRL_DIF_PHY_CNTL14, cntl14);
		lcd_phy_cntl_set_t5m(phy, status, 0, 1, ckdi);
	} else {
		lcd_phy_cntl_set_t5m(phy, status, 0, 0, 0);
	}
}

static void lcd_p2p_phy_set_t5m(struct aml_lcd_drv_s *pdrv, int status)
{
	unsigned int p2p_type, vcm_flag;
	struct p2p_config_s *p2p_conf;
	struct phy_config_s *phy = &pdrv->config.phy_cfg;
...
struct lcd_phy_ctrl_s lcd_phy_ctrl_t5m = {
	.ctrl_bit_on = 1,
	.lane_lock = 0,
	.phy_set_lvds = lcd_lvds_phy_set_t5m,
	.phy_set_vx1 = lcd_vbyone_phy_set_t5m,
	.phy_set_mlvds = lcd_mlvds_phy_set_t5m,
	.phy_set_p2p = lcd_p2p_phy_set_t5m,
	.phy_set_mipi = NULL,
	.phy_set_edp = NULL,
};
