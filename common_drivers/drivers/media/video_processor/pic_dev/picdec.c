			...
			struct config_para_ex_s *ge2d_config)
{
	...
	if (mem) {
		aml_pr_info(1, "assign mem_handle for new vfram\n");
		new_vf->mem_handle = mem;
	}
	vfbuf_use[index]++;
	       ...
	       struct config_para_ex_s *ge2d_config)
{
	...
	if (picdec_device.use_reserved) {
		...
		p = ioremap(cs0.addr, cs0.width * cs0.height);
		memset(p, 0, cs0.width * cs0.height);
		iounmap(p);
		p = ioremap(cs1.addr, cs1.width * cs1.height);
...
{
	int ret = 0;
	/*MAX_USER_RT_PRIO->MAX_RT_PRIO in kernel5.15*/
	...
	if (!picdec_device.use_reserved) {
		...
		if (picdec_device.output_format_mode) {
			if (picdec_device.cma_mode == 0) {
				...
				mem = codec_mm_alloc("picdec", 72 * SZ_1M, 0, flags);
				if (!mem)
					return 0;
				picdec_device.buffer_start = mem->phy_addr;
				/*
			...
			if (picdec_device.cma_mode == 0) {
				...
				mem = codec_mm_alloc("picdec", 56 * SZ_1M, 0, flags);
				if (!mem)
					return 0;
				picdec_device.buffer_start = mem->phy_addr;
