...
unsigned int hdmirx_audio_fifo_rst(void)
{
	...
	if (rx.chip_id >= CHIP_ID_T7) {
		if (rx.chip_id >= CHIP_ID_T5M) {
			hdmirx_wr_cor(RX_AUD_FIFO_RST, 0xff);
			udelay(1);
			hdmirx_wr_cor(RX_AUD_FIFO_RST, 0x0);
		}
...
int rx_set_port_hpd(u8 port_id, bool val)
{
	if (port_id < E_PORT_NUM) {
		if (val) {
			...
			if (rx.chip_id >= CHIP_ID_T7 && port_id == rx.port)
...
void hdmirx_set_video_mute(bool mute)
{
	if (rx.chip_id >= CHIP_ID_T5M) {
		if (rx.pre.colorspace == E_COLOR_RGB) {
			hdmirx_wr_top(TOP_OVID_OVERRIDE0, 0x0);
		} else {
			hdmirx_wr_top(TOP_OVID_OVERRIDE0, 0x80200);
			/* FRL mode */
			if (rx.chip_id >= CHIP_ID_T3X)
				hdmirx_wr_top(TOP_OVID_OVERRIDE2, 0x80200);
		}
		hdmirx_wr_bits_top(TOP_OVID_OVERRIDE0, _BIT(30), mute);
		hdmirx_wr_bits_top(TOP_OVID_OVERRIDE0, _BIT(30), mute);
	} else if (rx.chip_id >= CHIP_ID_T7 && rx.chip_id < CHIP_ID_T5M) {
