...
static int pre_dwc_rst_wait_cnt_max;
...
int clk_chg_cnt;
int clk_chg_max = 3;
static int hdr_err_cnt;
static int hdr_ok_cnt;
bool hdr_err_chk_en = 1;
...
void hdmirx_fsm_var_init(void)
{
	...
	case CHIP_ID_TM2:
		...
		pre_dwc_rst_wait_cnt_max = 1;
		spec_dev_wait_cnt_max = 400;
		clk_unstable_max = 50;
		esd_phy_rst_max = 16;
		pll_unlock_max = 30;
	...
	case CHIP_ID_T5M:
		...
		sig_stable_err_max = 5;
		sig_stable_max = 10;
		dwc_rst_wait_cnt_max = 30; //for repeater
		pre_dwc_rst_wait_cnt_max = 30;
		spec_dev_wait_cnt_max = 400;
		clk_unstable_max = 50;
		esd_phy_rst_max = 16;
...
struct spec_dev_info {
	u8 dev_vendor_id[VENDOR_ID_LEN];
	u32 wait_cnt;
};

struct spec_dev_info _spec_dev_info[DEV_CNT] = {
	{{0X0, 0X0, 0X0}, 150},
	{{0x08, 0x00, 0x46}, 400},
	{{0x00, 0x05, 0xcd}, 150},
};
...
int cec_set_vendor_id(u8 *vendor_id)
{
	...
	for (i = 0; i < DEV_CNT; i++) {
		if (!memcmp((char *)vendor_id, _spec_dev_info[i].dev_vendor_id, VENDOR_ID_LEN)) {
			rx.spec_vendor_id = true;
			spec_dev_wait_cnt_max = _spec_dev_info[i].wait_cnt;
			if (log_level & 0x1000)
				rx_pr("vendor id=%s\n", vendor_id);
		}
	}
...
bool is_unnormal_format(u8 wait_cnt)
{
	...
	rx_pr("hdcp: %d, state: %d\n", rx.hdcp.hdcp_version, rx.cur.hdcp22_state);
	if (rx.hdcp.hdcp_version == HDCP_VER_22 && (rx.cur.hdcp22_state == 0 ||
		rx.cur.hdcp22_state == 1)) {
...
void rx_get_global_variable(const char *buf)
{
	...
	pr_var(pre_dwc_rst_wait_cnt_max, i++);
	...
	pr_var(rx.aml_phy.hyper_gain_en, i++);
}
...
int rx_set_global_variable(const char *buf, int size)
{
	...
	if (set_pr_var(tmpbuf, var_to_str(pre_dwc_rst_wait_cnt_max),
						&pre_dwc_rst_wait_cnt_max, value))
		return pr_var(pre_dwc_rst_wait_cnt_max, index);
	...
	if (set_pr_var(tmpbuf, var_to_str(rx.aml_phy.hyper_gain_en),
		&rx.aml_phy.hyper_gain_en, value))
		return pr_var(rx.aml_phy.hyper_gain_en, index);
	return 0;
}
...
void hdmirx_open_port(enum tvin_port_e port)
{
	...
	vpp_mute_enable = 1;
	if ((rx.port == ((port - TVIN_PORT_HDMI0) & 0xf)) &&
		rx.resume_flag) {
		rx.resume_flag = false;
		return;
	} else {
		sm_pause = 1;
...
{
	rx.phy.timestap = ktime_get_real_seconds();//get_seconds();
}
...
void rx_main_state_machine(void)
{
	...
	case FSM_SIG_WAIT_STABLE:
		...
		if (rx.spec_vendor_id && !rx_is_specific_20_dev())
			dwc_rst_wait_cnt_max = spec_dev_wait_cnt_max;
		else
			dwc_rst_wait_cnt_max = pre_dwc_rst_wait_cnt_max;
		dwc_rst_wait_cnt++;
...
void rx_ui_edid_cec_monitor(void)
{
	int edid_s = 0;

	if (is_earc_hpd_low()) {
		if (log_level & EDID_LOG)
			rx_pr("do nothing for earc hpd low\n");
		return;
	}

	edid_s = rx_get_edid_size(rx_get_edid_index());

	if (ui_hdmi_cec_set &&
		edid_s == MAX_EDID_BUF_SIZE) {
		ui_hdmi_cec_set = false;
		if (hdmi_cec_en)
			rx_force_hpd_rxsense_cfg(1);
		/* fix source can't get edid if cec off */
		if (rx.boot_flag && !hdmi_cec_en) {
	...
	if (!rx.open_fg &&
		edid_s == MAX_EDID_BUF_SIZE &&
		!rx.ui_edid_cec_set) {
		rx_pr("ui edid cec set force hpd\n");
		rx_force_hpd_rxsense_cfg(1);
		rx.ui_edid_cec_set = true;
		rx.boot_flag = false;
	}
}

void rx_hdr_err_monitor(void)
{
	struct drm_infoframe_st *drmpkt;
	u8 eotf_tmp = 0xff;

	drmpkt = (struct drm_infoframe_st *)&rx_pkt.drm_info;
	if (!hdr_err_chk_en || rx.state != FSM_SIG_READY)
		return;
	if (drmpkt->des_u.tp1.eotf != eotf_tmp) {
		eotf_tmp = drmpkt->des_u.tp1.eotf;
		if (hdr_err_cnt++ == 3) {	
			reset_pcs();
			udelay(1);
			reset_pcs();
...
void hdmirx_timer_handler(struct timer_list *t)
{
	...
	rx_clkmsr_monitor();
        rx_hpd_monitor();
	rx_ui_edid_cec_monitor();
	if (rx.open_fg || rx.resume_flag) {
		rx_nosig_monitor();
		rx_cable_clk_monitor();
		rx_check_repeat();
