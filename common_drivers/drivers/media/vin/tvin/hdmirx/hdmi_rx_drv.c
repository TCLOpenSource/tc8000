...
void hdmirx_dec_close(struct tvin_frontend_s *fe)
{
	...
	if (rx.arc_port == rx.port && rx.state < FSM_SIG_STABLE) {
		rx_set_cur_hpd(1, 4);
		pre_port = 0xff;
	}
...
unsigned int hdmirx_get_hpd_info(void)
{
	...
	return (rx_get_hpd_sts() >> rx.port) & 1;
}
			     ...
			     struct tvin_sig_property_s *prop)
{
	...
	prop->skip_vf_num = vdin_drop_frame_cnt;
		...
		rx_pr("dvi:%#x,color[%d,%#x,%#x,%#x],fps:%d,spd[%#x,%#x],lat:[%#x,%#x,%#x],vic:%d,ec:%d\n",
