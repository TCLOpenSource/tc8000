				    ...
				    unsigned int buf_num)
{
	...
	if (highmem_flag == 0) {
		if (devp->mem_type == VDIN_MEM_TYPE_SCT) {
			vdin_dump_write_sct_mem_df(devp, buf_num, df);
		} else {
...
static void vdin_dump_state(struct vdin_dev_s *devp)
{
	...
	pr_info("0721,flags=0x%x,flags_isr=0x%x\n", devp->flags, devp->flags_isr);
...
start_chk:
	...
	} else if (!strcmp(parm[0], "power_control_en")) {
		if (parm[1] && (kstrtouint(parm[1], 0, &temp) == 0)) {
			devp->debug.power_control_en = temp;
			pr_info("power_control_en:%d\n", devp->debug.power_control_en);
		}
	} else {
