...
static void vdin_game_mode_check(struct vdin_dev_s *devp)
{
	...
	if (game_mode == 1 && (!IS_TVAFE_ATV_SRC(devp->parm.port))) {
		...
		if (devp->fmt_info_p->scan_mode == TVIN_SCAN_MODE_INTERLACED ||
			...
			(devp->parm.info.fps >= 40 && devp->parm.info.fps <= 144)) {
			if (cpu_after_eq(MESON_CPU_MAJOR_ID_TL1) && panel_reverse == 0) {
				devp->game_mode = (VDIN_GAME_MODE_0 |
					VDIN_GAME_MODE_1 |
					VDIN_GAME_MODE_SWITCH_EN);
...
{
	if (skip_frame_debug)
		pr_info("vdin.%d:irq_flag:%d %s [%d %d],WL:%x WR:%x RL:%x RM:%x\n",
...
irqreturn_t vdin_isr(int irq, void *dev_id)
{
	...
	int ret;
	static u64 cur_us, pre_us, interval_us;
	static unsigned int err_vsync;

	cur_us = ktime_to_us(ktime_get());
	interval_us = cur_us - pre_us;
	pre_us = cur_us;
	/* avoid null pointer oops */
	if (!devp)
		return IRQ_HANDLED;
	...
	devp->irq_cnt++;
	if (vdin_isr_monitor & VDIN_ISR_MONITOR_VF)
		pr_info("vdin irq_cnt:%u cur_time:%llu interval_us:%llu\n",
			devp->irq_cnt, ktime_to_us(ktime_get()), interval_us);
	...
	if (interval_us <= VDIN_INPUT_MAX_FPS)
		err_vsync++;
	else
		err_vsync = 0;
...
void vdin_vpu_clk_mem_pd(struct vdin_dev_s *devp, unsigned int on)
{
	...
	if (cpu_after_eq(MESON_CPU_MAJOR_ID_SC2) &&
		!devp->debug.power_control_en) {
		pr_info("vdin%d,donot control power domain on cpu:%#x\n",
			devp->index, get_cpu_type());
		return;
	}
	if (on) {
		if (offset)
