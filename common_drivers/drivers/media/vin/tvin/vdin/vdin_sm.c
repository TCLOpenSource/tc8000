...
static enum tvin_sg_chg_flg vdin_hdmirx_fmt_chg_detect(struct vdin_dev_s *devp)
{
	...
	if (sm_ops->get_sig_property) {
		...
		if (devp->pre_prop.avi_ec != devp->prop.avi_ec) {
			signal_chg |= TVIN_SIG_CHG_AVI_EC;
			pr_info("%s avi_ec chg:(0x%x->0x%x)\n", __func__,
				devp->pre_prop.avi_ec,
				devp->prop.avi_ec);
			devp->pre_prop.avi_ec = devp->prop.avi_ec;
	...
	if ((devp->flags & VDIN_FLAG_DEC_STARTED) &&
		...
		if (pre_vs != cur_vs || pre_ve != cur_ve ||
			...
			if (sm_debug_enable & VDIN_SM_LOG_L_4)
				pr_info("[smr.%d] pre_vs(%d->%d),pre_ve(%d->%d),pre_hs(%d->%d),pre_he(%d->%d),cut_window_cfg:0x%x\n",
...
void tvin_sig_chg_event_process(struct vdin_dev_s *devp, u32 chg)
{
	...
	if (chg & TVIN_SIG_CHG_STS) {
		...
		} else if (chg & TVIN_SIG_CHG_AVI_EC) {
			devp->event_info.event_sts = TVIN_SIG_CHG_AVI_EC;
		...
		if (sm_ops->nosig(devp->frontend)) {
			...
			if (sm_ops->fmt_changed(devp->frontend)) {
				...
				if (IS_TVAFE_ATV_SRC(port) &&
					...
					unstable_in = other_unstable_in_cnt;
