...
#define VENDOR_ID_DEV	3
u8 dev_vendor_id[VENDOR_ID_DEV][3] = {
	{0, 0, 0},
	{0x08, 0x00, 0x46},
	{0x00, 0x05, 0xcd},
};
...
bool cec_message_op(unsigned char *msg, unsigned char len)
{
	...
	case 0x47:
		...
		break;
	case 0x87:
		for (j = 0; j < VENDOR_ID_DEV; j++) {
			for (i = 2; i < len; i++) {
				if (msg[i] != dev_vendor_id[j][i - 2])
					break;
			}
			if (i == len) {
			#ifdef CONFIG_AMLOGIC_MEDIA_TVIN_HDMI
				cec_set_vendor_id(&(dev_vendor_id[j][0]));
			#endif
				CEC_ERR("specific vendor id:%d", dev_vendor_id[j][0]);
			}
		}
		/* verdor ID */
		break;
...
int __attribute__((weak))cec_set_vendor_id(uint8_t *vendor_id)
{
	return 0;
}
