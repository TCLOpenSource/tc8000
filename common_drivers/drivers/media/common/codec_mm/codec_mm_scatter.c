...
static inline int codec_mm_scatter_align_count(int start_id)
{
	...
	if (scatter_alloc_align_size)
		tail_pages_start = ALIGN(start_id, PAGE_COUNT(scatter_alloc_align_size * SZ_1M));
...
codec_mm_slot_alloc(struct codec_mm_scatter_mgt *smgt, int size, int flags)
{
	...
	do {
		...
		if ((try_alloc_size <= 0 ||
			...
			if (scatter_alloc_align_size) {
				if (try_alloc_size < scatter_alloc_align_size * SZ_1M) {
					DBG_LOG("No SCATTER_GRANULARITY memory can be used\n");
					break;
				}
				if (try_alloc_size % (scatter_alloc_align_size * SZ_1M) != 0) {
						try_alloc_size =
					ALIGN_DOWN(try_alloc_size,scatter_alloc_align_size * SZ_1M);
				}
			}
				      ...
				      int want_pages)
{
	...
	int ret;
	u64 startus;
	int tail_pages_start = codec_mm_scatter_align_count(want_pages);

	pr_dbg("%s tail_pages_start %d start_id %d\n",
		__func__, tail_pages_start, want_pages);

	if (!mms)
		return -1;
	smgt = (struct codec_mm_scatter_mgt *)mms->manager;
	startus = codec_mm_get_current_us();
	ret = codec_mm_scatter_alloc_want_pages_in(smgt, mms, tail_pages_start);
	codec_mm_update_alloc_time(smgt, startus);
	return ret;
}
...
static struct mconfig codec_mm_sc_configs[] = {
	...
	MC_PU32("scatter_alloc_align_size", &scatter_alloc_align_size),
