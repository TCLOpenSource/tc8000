...
static void irq_clr_c3(void)
{
	...
	data32 = osd_reg_read(VPU_VOUT_IRQ_CTRL);
	data32 |= 0x01;
	osd_reg_write(VPU_VOUT_IRQ_CTRL, data32);
}
...
static void osd_update_enable(u32 index)
{
	...
	if (osd_hw.osd_meson_dev.afbc_type == MESON_AFBC) {
		...
		if (osd_hw.osd_afbcd[index].enable == ENABLE &&
			...
			osd_log_dbg(MODULE_BLEND, "%s: line=%d osd%d, osd_afbcd.enable=%d, osd_hw.enable=%d, dim_layer=%d\n",
				__func__, __LINE__,
				index, osd_hw.osd_afbcd[index].enable,
				osd_hw.enable[index], osd_hw.dim_layer[index]);

			osd_log_dbg(MODULE_BLEND,"%s: osd%d, afbc_start=%d, phy_addr=0x%x\n",
				__func__, index, osd_hw.osd_afbcd[index].afbc_start,
				osd_hw.osd_afbcd[index].phy_addr);
...
{
	if (osd_hw.osd_meson_dev.cpu_id == __MESON_CPU_MAJOR_ID_T5M)
...
#define RDMA_DETECT_REG VIU_OSD2_TCOLOR_AG0
	...
	osd_log_dbg(MODULE_RENDER,
		...
		__func__, get_rdma_irq_done_line(VIU1), line1, line2, total_line);
	osd_wait_vsync_hw_viux(output_index);
