...
int lut3d_order = 2;/* 0 RGB 1 GBR 2 BGR for top data*/
int lut3d_debug;
...
static void _update_lut3d_data(unsigned int p3dlut_in[][3])
{
	int d0, d1, d2;
...
void lut3d_update(unsigned int p3dlut_in[][3])
{
	...
	_VSYNC_WR_MPEG_REG(VPP_LUT3D_CBUS2RAM_CTRL, 1);
	_VSYNC_WR_MPEG_REG(VPP_LUT3D_RAM_ADDR, 0 | (0 << 31));
	for (i = 0; i < 17 * 17 * 17; i++) {
		//{comp0, comp1, comp2}
		_VSYNC_WR_MPEG_REG(VPP_LUT3D_RAM_DATA,
			      ((plut3d[i * 3 + 1] & 0xfff) << 16) |
			      (plut3d[i * 3 + 2] & 0xfff));
		_VSYNC_WR_MPEG_REG(VPP_LUT3D_RAM_DATA,
			      (plut3d[i * 3 + 0] & 0xfff)); /*MSB*/
	...
	_VSYNC_WR_MPEG_REG(VPP_LUT3D_CBUS2RAM_CTRL, 0);
}
...
void dump_using_plut3d_table(void)
{
	unsigned int i;

	if (!plut3d) {
		pr_info("[lut] No lut3d table.\n");
		return;
	}

	pr_info("*****dump lut3d table:*****\n");

	pr_info("[lut] lut3d_order = %d\n", lut3d_order);

	for (i = 0; i < 17 * 17 * 17; i++)
		pr_info("[lut] %d: %03x %03x %03x\n",
			i,
			plut3d[i * 3 + 0],
			plut3d[i * 3 + 1],
			plut3d[i * 3 + 2]);
