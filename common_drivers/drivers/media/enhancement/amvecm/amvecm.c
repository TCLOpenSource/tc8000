...
static void vpp_backup_histgram(struct vframe_s *vf)
{
	...
	for (i = 0; i < 64; i++)
		vpp_hist_param.vpp_dark_hist[i] = vf->prop.hist.vpp_dark_hist[i];
...
{
	if (vecm_latch_flag2 & (CT_UPDATE | LUT3D_UPDATE))
		lut3d_set_api();
}
				 ...
				 unsigned int *hdr_tm)
{
	...
	if (hdr_tm) {
		...
		} else if (lut_type & SDR_LUT) {
			for (i = 0; i < length; i++)
				oo_y_lut_sdr2hdr_tv[i] = hdr_tm[i];

			vecm_latch_flag |= FLAG_SDR_OOTF_LATCH;

			if (debug_amvecm & 4) {
				for (i = 0; i < length; i++) {
					pr_info("oo_y_lut_sdr2hdr_tv[%d] = %d",
						i, oo_y_lut_sdr2hdr_tv[i]);
					if (i % 8 == 0)
						pr_info("\n");
				}
				pr_info("\n");
...
static void cm_init_config(int bitdepth)
{
	...
	if (chip_type_id == chip_s5) {
		...
		for (i = 0; i < 32; i++) {
			...
			/*T5M add final gain*/
			if (chip_type_id == chip_t5m) {
				WRITE_VPP_REG(VPP_CHROMA_ADDR_PORT, CM_FINAL_GAIN_LUMA);
				WRITE_VPP_REG(VPP_CHROMA_DATA_PORT, 0x800);
				WRITE_VPP_REG(VPP_CHROMA_ADDR_PORT, CM_FINAL_GAIN_SAT);
				WRITE_VPP_REG(VPP_CHROMA_DATA_PORT, 0x800);
				WRITE_VPP_REG(VPP_CHROMA_ADDR_PORT, CM_FINAL_GAIN_HUE);
				WRITE_VPP_REG(VPP_CHROMA_DATA_PORT, 0x800);
			}
		}
	}
				  ...
				  const char *buf, size_t count)
{
	...
	if (!strncmp(parm[0], "vpp_size", 8)) {
		...
		if (!strcmp(parm[1], "init_tab"))
			dump_plut3d_table();
		else if (!strcmp(parm[1], "using_tab"))
			dump_using_plut3d_table();
...
{
	if (!plut3d)
		plut3d = vmalloc(14739 * sizeof(int));
