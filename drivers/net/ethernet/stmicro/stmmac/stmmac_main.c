...
static void disable_eth_irq(void);
...
static int eth_relax_en = 1;
module_param(eth_relax_en, int, 0644);

static void disable_eth_irq(void)
{
	pr_emerg("%s eth_irq=%d eth_relax_en=%d \n", __func__, eth_irq, eth_relax_en);

	if (eth_irq && eth_relax_en) {
		pr_emerg("disable eth_irq nosync\n");
		disable_irq_nosync(eth_irq);
	}
}
		     ...
		     struct stmmac_resources *res)
{
	...
	pr_emerg("eth_irq = %d,%d,%d\n", eth_irq, res->wol_irq, res->lpi_irq);
	eth_irq = res->irq;
